...
local function print_sample (g, n)
   io_write("\n")
   if g == nil then
      io_write("Ooops, we just ran into panic code "
                  ..tonumber(gb.panic_code).."!\n")
      if tonumber(gb.io_errors) ~= 0 then
         io_write("(The I/O error code is 0x"..tonumber(gb.io_errors).."\n")
      end
   else
...
io_write("GraphBase samples generated by test_sample:\n")

local g = gb.random_graph(3, 10, 1, 1, 0, nil, dst, 1, 2, 1)
local gg = gb.complement(g, 1, 1, 0)
local v = ffi_cast("Vertex*", sgb.gb_typed_alloc(1, "Vertex", gg.data))
v.name = sgb.gb_save_string("Testing")
gg.util_types[10] = byte("V")
gg.ww.V = v
sgb.save_graph(gg, "test.gb")
gb.gb_recycle(g)
gb.gb_recycle(gg)
...
print_sample(sgb.book("homer", 500, 400, 2, 12, 10000, -123456, 789),81)
